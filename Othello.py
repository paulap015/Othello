class Othello():
    ...
    def generate_possible_moves(self, move, board):
        movimientos = []
        for key in self.getBoard().keys():
            if self.getBoard()[key] == ' ' and self.validate_rules(key, move, True):
                movimientos.append(key)
        return set(movimientos)
    def report(self):
        return self.contarFichas('X'), self.contarFichas('O')
    def insert_move(self, pos, move, validate):
        if(self.__board[pos] == ' '):
            if(self.validate_rules(pos, move, False)):
                ...
                if validate == False:
                    print()
                    self.print_board()
                    scoreIa, scoreHuman = self.report()
                    print("Score IA: ", scoreIa, " Score Human: ", scoreHuman) 
            elif validate == False:
    ...
    def validate_rules(self, pos, move, validate):
        result = (self.validate_col(pos, move, validate), self.validate_row(
            pos, move, validate), self.validate_diag(pos, move, validate))
        return any(result)
    ...
    def validate_diag(self, pos, move, isValid):
        ...
        if pos not in self.__border["Left"] and pos not in self.__border["Right"] and pos not in self.__border["Top"] and pos not in self.__border["Low"]:
            ...
            firstDiag = (self.__board[pos-(self.__board_size+1)] ==
                         ' ', self.__board[pos+(self.__board_size+1)] == ' ')
            secondDiag = (self.__board[pos+(self.__board_size-1)] ==
                          ' ', self.__board[pos-(self.__board_size-1)] == ' ')
            posicionesVacias = (self.__board[pos-(self.__board_size+1)] == ' ', self.__board[pos+(self.__board_size+1)]
                                == ' ', self.__board[pos+(self.__board_size-1)] == ' ', self.__board[pos-(self.__board_size-1)] == ' ')
            ...
            if((self.__board[pos-(self.__board_size+1)] == move and (firstDiag[1] == True or self.__board[pos+(self.__board_size+1)] == move)) or
               (self.__board[pos+(self.__board_size+1)] == move and (firstDiag[0]
                                                                     == True or self.__board[pos-(self.__board_size+1)] == move))
               ):
                movFirstDiag = False
            if((self.__board[pos+(self.__board_size-1)] == move and (secondDiag[1] == True or self.__board[pos-(self.__board_size-1)] == move)) or
               (self.__board[pos-(self.__board_size-1)] == move and (secondDiag[0]
                                                                     == True or self.__board[pos+(self.__board_size-1)] == move))
               ):
        ...
        if isBorder == False:
            ...
            # es borde izquierdo debe buscar derSuperior derInfe
            if (pos in self.__border["Left"] and (pos not in self.__border["Top"] and pos not in self.__border["Low"])):
